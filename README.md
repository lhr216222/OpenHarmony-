...
为了验证本文提出方法的有效性，设计一种基于DNS协议的存储型隐蔽通信程序。通常防火墙配置为允许 UDP 端口 53 上的所有数据包，即 DNS 流量通常允许通过企业防火墙而无需深度检查或状态维护。DNS 轻易穿透防火墙的特点使其成为隐蔽通信的极佳载体。除此之外，DNS 分为查询请求和查询响应，方便实现可靠的双向通信。

该隐蔽通信方法的原理是利用DNS协议的flags字段作为存储型隐蔽信道，将秘密信息嵌入其中，随着正常的DNS查询请求和响应报文在网络中传输，从而实现隐蔽信息的传递。具体来说，flags 字段中，Opcode、AA、TC、RD、RA 字段长度为 8 位，将秘密信息拆分成一个个字节，将其隐藏在这几个字段。DNS的数据链路层协议是UDP协议，UDP协议不具备可靠的端到端通信能力， 因此手动实现了一个简单的协议来提高数据传输中的可靠性。其工作过程为： 

1、客户端向服务器发送一个 DNS 查询请求，其中字符‘s’被嵌入到标志字段中。 

2、服务器创建一个具有新 IPv4 id 值的新数据包，并使用与查询请求相同的DNS id 生成 DNS 响应，秘密信息的第一个字符被嵌入到 DNS 的标志字段。 

3、客户端识别 DNS 响应并提取第一个字符。客户端通过生成一个新的 DNS查询请求来确认字符的接收，该查询请求的 IPv4 id 字段设置为刚接收的数据包的 IPv4 id+1 值。 

4、有了 id+1，服务器可以确定数据包的传递。并传递下一个数据包。 

5、通信在 3，4 循环，直到消息传输结束，服务器发送一个‘~’字符，通知客户端消息发送结束。

图13 时间型隐蔽信道检测

通信内容设置为“This is a secret message!”，数据包发送间隔为0.2s，设置通讯设备的IP地址、AES密钥等信息。运行程序，程序成功通过隐蔽信道传输数据，传输结果和捕获的DNS报文如下图所示。

B. 隐蔽通信检测与拦截模拟实验

由于 OpenHarmony 系统开发环境对硬件和软件配置有一定要求，且其内核架构和网络模块的实际部署涉及大量底层开发与调试工作，开展真实环境下的隐蔽通信检测与拦截实验存在较高的技术复杂性。因此，为了更加高效地验证设计方法的可行性与效果，本研究选择在 Windows 平台上进行模拟实验。通过构建与 OpenHarmony 网络模块功能类似的实验环境，可以在不依赖真实硬件的情况下实现隐蔽通信检测与拦截逻辑的验证。尽管模拟实验无法完全复现真实环境下的性能表现，但它为后续的实际部署提供了理论支持和技术验证，是研究初期的重要过渡手段。

对于DNS协议的Opcode、AA、TC、RD、RA等字段，我们有如下先验知识：大多数 DNS 查询会使用标准查询操作（Opcode = 0），Opcode = 0（标准查询）约占 99% 以上的 DNS 查询；大多数正常的 DNS 响应（特别是来自权威 DNS 服务器的）会有 AA = 1，这个比例可以高达 90% 以上；大多数正常的 DNS 响应不会被截断，TC = 0 占绝大多数 DNS 响应，可能超过 99%；正常情况下，大多数客户端会希望 DNS 服务器进行递归查询，大多数 DNS 查询会将 RD 设置为 1，通常超过 95%；对于大多数 DNS 服务器，RA = 1 的比例很高，通常会达到 90% 以上。对于隐蔽通信，可能不符合上述先验，图14展示了隐蔽通信情况下各字段的统计结果。

在检测实验中，以30s为时间窗口，我们从 DNS 查询包中提取五个关键特征，并将其表示为一个五维向量，如表2所示。由于缺乏训练数据，因此根据经验人工设计分类器，如下式所示。当大于阈值时，则特征被分类为异常。

模拟实验的结果如图15所示。实验结果表明，所提出的隐蔽通信检测与拦截方法在模拟环境下能够有效地识别和阻断基于DNS协议的隐蔽通信行为。实验中，系统能够实时捕获网络流量，准确解析DNS数据包，并基于特征向量计算结果快速做出响应。我们的分类器能够有效地识别出异常的DNS通信行为。具体来说，我们得到的特征向量为f=[0.05714285714285714,0.6571428571428571,0.5142857142857142, 0.4857142857142857, 0.42857142857142855]，这个向量与正常通信的特征分布存在显著差异，触发了我们的拦截与警报。这不仅验证了方法的可行性，也为进一步的优化和实际部署提供了宝贵的数据支持。

尽管模拟实验无法完全复现真实环境下的性能表现，但实验结果为后续的实际部署提供了理论支持和技术验证。未来的工作将集中在开发实际可用的模块并将其集成到OpenHarmony系统中，以及进一步提高检测的准确性和效率，将该方法应用于更广泛的网络环境和协议中。
## 总结
本文针对OpenHarmony 操作系统在隐蔽通信检测与防御方面的不足，提出了一种基于系统级实现的隐蔽通信检测与拦截方案。该方案通过在内核层和子系统层设计防护模块，对OpenHarmony 系统中涉及通信的关键模块进行分析，并设计相应的检测与拦截机制。通过对网络流量的捕获、特征提取和模式识别，系统能够有效地识别和阻断存储型和时间型隐蔽通信行为。模拟实验结果表明，该方案能够有效地识别和拦截基于DNS 协议的隐蔽通信行为，为OpenHarmony 系统的安全防护提供了新的思路和方法。未来工作将集中于将方案集成到OpenHarmony 系统中，进行实际部署和测试，验证其在真实环境下的性能和效果，并进一步优化检测算法和拦截机制，提高检测的准确性和效率，将其应用于更广泛的网络环境和协议中，以提升物联网设备的安全性和可靠性，为万物互联的未来提供更安全的保障。
